...
public class JsonAD {
	...
	while (symb.sym != sym.EOF){
		...
		case 1:                                     
		    switch(symb.sym) {
		    case sym.ENDOBJECT:                      // ce qui suit le point peut se dériver en le mot vide, jusque end-array
		...
		case 0:                                      // Value -> . number 
			...
			break;
		    case sym.BEGINOBJECT:
			newrule=new RuleNode(rule.OBJECT, 0);// on crée un noeud Value
			current.appendChild(newrule);       // que l'on ajoute à la fin du noeud courant
			current=newrule;                    // et on descend dans l'arbre
			break;
		    case sym.BEGINARRAY:  
			newrule=new RuleNode(rule.ARRAY, 0);// on crée un noeud Value
			current.appendChild(newrule);       // que l'on ajoute à la fin du noeud courant
			current=newrule;                    // et on descend dans l'arbre
			break; 
