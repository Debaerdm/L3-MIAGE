    ...
    while (symb.sym != sym.EOF){
      if (symb.sym == sym.LT) {
        ...
        if (symb.sym == sym.NAME) {
          ...
          do {
            yy.yybegin(yy.ATTRIBUTE);
            symb = yy.next_token();
            if(symb.sym == sym.ATTRIBUT) {
              System.out.println("Attribut : "+ symb.value);	
             }
          } while ((symb.sym != sym.SLASHGT) && (symb.sym != sym.GT));
          if (symb.sym == sym.SLASHGT) {
            System.out.println("End element");
          } else {
            yy.yybegin(yy.INELEMENT);
            symb = yy.next_token();
            if(symb.sym == sym.CONTENT && symb.value != null) {
                System.out.println("Contenue : "+symb.value);
            }
          }
        }
      ...
      else  { symb = yy.next_token();  yy.yybegin(yy.YYINITIAL); }
      //System.out.println(yy.yystate());
    }
